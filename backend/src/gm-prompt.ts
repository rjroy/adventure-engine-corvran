...
export function createGMMcpServerWithCallbacks(callbacks: GMMcpCallbacks) {
  return createSdkMcpServer({
    ...
    version: "4.0.0",
    tools: [
      createSetThemeTool(callbacks.onThemeChange),
      createSetXpStyleTool(callbacks.onXpStyleChange),
      createSetCharacterTool(callbacks.onSetCharacter),
      createSetWorldTool(callbacks.onSetWorld),
      createListCharactersTool(callbacks.onListCharacters),
      createListWorldsTool(callbacks.onListWorlds),
    ],
  });
}
