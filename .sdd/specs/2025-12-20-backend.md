...
- **REQ-F-31a**: Background images must be stored with filename convention: `{mood}-{genre}-{region}-{timestamp}.png` for automatic catalog discovery via glob search
...
### Claude Agent SDK File Access

- **REQ-F-41a**: System shall enable Claude Agent SDK file tool access to markdown files in playerRef and worldRef directories (world_state.md, locations.md, characters.md, player.md, quests.md) for narrative state persistence
