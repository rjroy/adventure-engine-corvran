...
import { createMockQueryFn, createThrowingQueryFn, textDelta, successResult, errorResult, assistantWithToolUse, userWithToolResult, } from "./helpers/mock-query.js";
const ADVENTURES_ROOT = "/test/adventures";
function buildTestApp(files, queryFn) {
...
describe("POST /adventures/:id/message", () => {
    test("returns 400 for empty body", async () => {
        ...
        const { app } = buildTestApp({ [`${ADVENTURES_ROOT}/quest/character.md`]: "Hero" }, queryFn);
    ...
    test("appends player message and GM response to history", async () => {
        ...
        const { app, fileOps } = buildTestApp({ [`${ADVENTURES_ROOT}/quest/character.md`]: "Hero" }, queryFn);
        const res = await app.request("/adventures/quest/message", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ message: "Hello" }),
        ...
        expect(history).toBe("**Player:** Hello\n\n**GM:** Welcome!\n\n");
    });
    ...
    test("AbortError appends partial text to history", async () => {
        ...
        const queryFn = createThrowingQueryFn([textDelta("Partial response")], abortError);
        const { app, fileOps } = buildTestApp({ [`${ADVENTURES_ROOT}/quest/character.md`]: "Hero" }, queryFn);
        const res = await app.request("/adventures/quest/message", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    test("fresh file read between requests reflects edits (REQ-MVP-17)", async () => {
        ...
        fileOps.getStore().set(`${ADVENTURES_ROOT}/quest/history.md`, "**Player:** Edited history\n\n");
    ...
    test("works with adventure that has no character or world", async () => {
        ...
        const { app } = buildTestApp({ [`${ADVENTURES_ROOT}/empty/placeholder`]: "" }, queryFn);
